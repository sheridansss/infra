...
# Бэкапим только сервисы, включённые в COMPOSE_PROFILES (.env) — состав
...
if [ "${#SERVICES[@]}" -eq 0 ]; then
  echo "в COMPOSE_PROFILES нет ни одного сервиса с данными — бэкапить нечего" >&2
  exit 1
fi
want() { [[ " ${SERVICES[*]} " == *" $1 "* ]]; }
...
# Выгрузка во внешнее хранилище, если настроена (BACKUP_REMOTE в .env) —
# при её ошибке бэкап уже лежит на диске, но скрипт вернёт ненулевой код,
# чтобы проблема была видна в journalctl.
bash backup-sync.sh
